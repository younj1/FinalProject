...
# Display columns in data2 to debug the issue
st.markdown("### Columns in the Healthcare Dataset (data2):")
st.write(data2.columns)

# Check if 'health_condition' exists in data2
if 'health_condition' not in data2.columns:
    st.error("The 'health_condition' column is not found in the healthcare dataset (data2). Please check the column name.")
else:
    ...
    with tab4:
        ...
        grouped_bar = px.bar(
            ...
            labels={"charges": "Average Charges", "children": "Number of Children", "smoker": "Smoker Status"},
            template=template,
        )
